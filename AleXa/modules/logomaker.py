...
async def lego(event):
 ...
 try:
    ...
    fname2 = "AleXaLogo.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @TheAleXaBot")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Go to Help From  @AnkiSupport_Official, {e}')
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("AleXa/resources/*")
    ...
    if os.path.exists(font_):
        if not font_.startswith("AleXa/resources/"):
            os.remove(font_)    
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./AleXa/resources/pandabg.png')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 25
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./AleXa/resources/font.otf", 170)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./AleXa/resources/spiderbg.png')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 25
    fillcolor = "gold"
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./AleXa/resources/spiderbg (2).png')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 25
    fillcolor = "gold"
